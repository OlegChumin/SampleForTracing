...
/**
 * Проверяет consumer событий заказа в сервисе расчёта цены.
 */
class PricingOrderEventConsumerTests {

    private final KafkaTracingService kafkaTracingService = mock(KafkaTracingService.class);
    private final PricingOrderEventConsumer consumer = new PricingOrderEventConsumer(new ObjectMapper(), kafkaTracingService);
    ...
    void handleOrderCreatedAcceptsValidPayload() {
        ...
        when(kafkaTracingService.startConsumerSpan(consumerRecord, "kafka checkout.order-created pricing"))
